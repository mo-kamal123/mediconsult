...
export default function Dropdown({
  ...
  const isObjectData =
    data.length > 0 && typeof data[0] === 'object' && (data[0].label || data[0].title);
  ...
  const getDisplayText = () => {
    ...
    if (isObjectData) {
      ...
      return matched ? matched.label || matched.title : placeholder;
    }
    ...
    >
      ...
      {type === 'search' && (
        <li
          className={`px-4 py-2 cursor-pointer hover:bg-gray-100 ${
            selected === 'all' ? 'bg-main text-white' : ''
          }`}
          onClick={() => handleSelect({ value: 'all', label: 'All' })}
        >
          All
        </li>
      )}
      ...
      {data.length > 0 ? (
        data.map((item, index) => (
          ...
          >
            {isObjectData ? item.label || item.title : item}
          </li>
        ))
      ) : (
